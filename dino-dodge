...
struct Obstacle{
	...
	bool erase;
	bool go;
    bool cactus_obs_type; // If obstacle type is a cactus, true. If obstacle type is a pterodactyl, false.
	int height;
	int width;
	int x_loc_prev;
	int y_loc_prev;
...
int main(void)
{
    ...
    *(pixel_ctrl_ptr + 1) = (int) &Buffer2;
	...
	struct Obstacle pterodactyl0 = {false, false, false, false, 30, 30, 0, 0, X_MAX, 90, BLACK};
	
	struct Obstacle Game_Obstacles[] = {cactus0, pterodactyl0, cactus1, cactus2};
    ...
    while (1)
    {
		...
		for(int i=0; i<num_obstacles; i++){
			...
			// New Obstacle Entering Screemn
			if(i == 0){ // In case we are handling first cactus
				Game_Obstacles[i].go = true;
			}
			else{
				cacti_dist = Game_Obstacles[i].x_loc_cur - Game_Obstacles[i-1].x_loc_cur;
...
void wait_for_vsync() {
	...
	*pixel_ctrl_ptr = 1;			// pixel control pointer contains the value 1
	...
	int status;
	status = *(pixel_ctrl_ptr + 3);	// status register has the pixel control pointer address offset by 2
	
	while((status & 1) != 0){		// status & 1 masks to bit zero (s bit)
		status = *(pixel_ctrl_ptr + 3);		// polling s bit until status is ready to write again
	}
}	
...
void draw_dino(struct Dino my_dino) {
	...
	for(int i=0; i<my_dino.height; i++){
		draw_line(x_loc, y_loc+i, x_loc + my_dino.width, y_loc+i, my_dino.colour);
	}
}
...
void init_timer() {
	...
	*(timer_p + 2) = TIMER_DELAY & 0xFFFF; // writing lower 16 bits of TIMER_DELAY to offset 8 of timer
	*(timer_p + 3) = (TIMER_DELAY >> 16) & 0xFFFF; // writing upper 16 bits of TIMER_DELAY to offset 12 of timer
	
	*(timer_p + 1) = 0b110; // start = 1, cont = 1 to begin the timer (110 where cont is bit 1 and start is bit 2)
}	

void display_timer_HEX(int timer) {
	
	char to_7seg[16] = {~0b1000000, ~0b1111001, ~0b0100100, ~0b0110000, ~0b0011001, ~0b0010010,
						~0b0000010, ~0b1111000, ~0b0000000, ~0b0011000, ~0b0001000, ~0b0000011, 
						~0b1000110, ~0b0100001, ~0b0000110, ~0b0001110}; // using char because 7 bits
	...
	*hex_p = 0; // turns off HEX5 and HEX4
	...
	for(int i=0; i<4; i++){ 	// saying 4 because 3600 (1 hour) is 4 digits big
		
		int val = timer % 10;
		val = to_7seg[val] & 0xFF; // converts val to a seven-seg value// get rid of & 0xFF if you manually invert
		
		val = val << (i*8); // shifts the value to put it in the proper place in hex_data (line 64)
